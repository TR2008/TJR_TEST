...
def adicionar_ao_cesto():
    ...
    # Normalize id (accepts strings sent by template)
    try:
    ...
    # Useful fallback for tests: if no product in DB, accept name+price in body
    if not produto:
        name = data.get('name')
    ...
    produto_key = str(produto_id)  # key in session (maintains correspondence with JS)
    basket = get_basket()
...
def checkout():
    ...
    order = {
        ...
        'user': session.get('utilizador_id'),
        'items': basket,
        'summary': summary
    }

    # In production: save to DB (Order model). Here we just store in session.
